##---------------------------------------------------------------------------##
##  File:
##      %Z% %Y% $Id: Util.pm,v 1.7 1997/08/27 21:01:25 ehood Exp $  %Z%
##  Author:
...
SGML::Util - miscellaneous Perl utility functions for SGML processing
...
B<SGML::Util> contains miscellaneous utility functions related
to SGML processing.  The following functions are exported by
B<SGML::Util>:

=head2 SGMLparse_attr_spec()

B<SGMLparse_attr_spec> takes a attribute specification string and
parses it into a list of name/value pairs.
...
A scalar string representing the SGML attribute specificaion list.
...
An array of name value pairs.  The calling routine can assign the
return value to a hash to allow easy access to attribute values.
For example:

    %attributes = SGMLparse_attr_spec($attribute_spec);
...
The I<stago>, I<gi>, and I<etago> should NOT be in the specification list
string.
...
Attribute values w/o a name are given a bogus name of the reserved
name indicator ('C<#>' in the reference concrete syntax) with a number
appended (eg. "C<#4>").  This is to handle the case when SHORTTAG is YES.

=item *

Any non-whitespace character is treated as a name character.
This allows the parsing of SGML-like markup.  For example, the
following will not generate a complaint:
...
=head2 SGMLattr_to_sgml()

B<SGMLattr_to_sgml> is the inverse operation of B<SGMLparse_attr_spec>.
It takes a attribute structure and generates the SGML markup
representation.
...
A reference to a hash or an array.  If a hash, the keys represent the
...
A string containing the SGML representation of the attributes.
...
Attribute names starting with the reserved name indicator ('C<#>' in the
reference concrete syntax) are skipped with only their values printed.
This is to handle the case when SHORTTAG is YES.
...
B<SGMLopen_lit> checks if a string has a literal that is not
closed.  I.e. If there is a quote without a matching quote,
the routine will return true.
