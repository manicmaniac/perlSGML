##---------------------------------------------------------------------------##
##  File:
##      %Z% $Id: Parser.pm,v 1.11 1997/09/11 17:40:38 ehood Exp $  %Z%
##  Author:
...
=head2 B<parse_data>(I<$fh>, I<$label>, I<$init_buf>, I<$line_no>)
...
=head2 B<comment_decl>(I<\@comments>)
...
=head2 B<ignored_data>(I<$data>)

B<ignored_data> is called for data that is in an IGNORE
marked section.

=head2 B<marked_sect_close>()

B<marked_sect_close> is called when a marked section close is
encountered.

=head2 B<marked_sect_open>(I<$status_keyword>, I<$status_spec>)

B<marked_sect_open> is called when a marked section open is
encountered.  The I<$status_keyword> argument is the status
keyword for the marked section (eg. INCLUDE, IGNORE).  The
I<$status_spec> argument is the original status specification text.
This may be equal to I<$status_keyword>, or contain an parameter
...
=head2 B<parm_entity_ref>(I<$name>)
...
=head2 B<processing_inst>(I<$data>)

B<processing_inst> is called for processing instructions.  B<$data>
is the content of the processing instruction.

=head2 B<start_tag>(I<$gi>, I<$attr_spec>)

B<start_tag> is called for start tags.  I<$gi> is the
generic indentifier of the start tag.  I<$attr_spec> is the
